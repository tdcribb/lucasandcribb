 ...
 */
  ...
  class BF_Admin_Page_Class {
    ...
    public function plupload_head_js(){
      if ($this->has_field('plupload')){
         $plupload_init = array(
            ...
            'filters' => array(array('title' => __('Allowed Files', 'flexible-frontend-login' ), 'extensions' => '*')),
            'multipart' => true,
    ...
    public function edit_insert_to_post_text( $safe_text, $text ) {
      if( is_admin() && 'Insert into Post' == $safe_text){
        if (isset($_REQUEST['apc']) && 'insert_file' == $_REQUEST['apc'] )
          return str_replace(__('Insert into Post', 'flexible-frontend-login' ), __('Use this File', 'flexible-frontend-login' ), $safe_text);
        else
          return str_replace(__('Insert into Post', 'flexible-frontend-login' ), __('Use this Image', 'flexible-frontend-login' ), $safe_text);
      }
      return $text;
    }
    ...
    /**
      ...
      public function DisplayPage() {
        ...
        if ($this->saved_flag)
          echo '<div class="alert alert-success"><p><strong>'.__('Settings saved.', 'flexible-frontend-login' ).'</strong></p></div>';
      ...
      echo '</div><div style="clear:both"></div><div class="footer_wrap">
          <div style="float:right;margin:32px 0 0 0">
            <input type="submit" style="margin-left: 25px;" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn" value="'.esc_attr(__('Save Changes', 'flexible-frontend-login' )).'" />
            <br><br>
          </div>
          <br style="clear:both"><br>
        </div>';
  ...
  public function insert_images() {
    ...
    foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
      ...
      //$li   .= "<a title='" . __( 'Delete this image', 'flexible-frontend-login'  ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$post_id}|{$id}|{$attachment_id}'>" . __( 'Delete', 'flexible-frontend-login'  ) . "</a>";
      $li   .= "<a title='" . __( 'Delete this image', 'flexible-frontend-login'  ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$post_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . __( 'Delete', 'flexible-frontend-login'  ) . "' /></a>";
  ...
  */
  public function wp_ajax_delete_image() {
    ...
    if ( $ok ){
      ...
      echo json_encode(array('message' => __( 'Cannot delete file. Something\'s wrong.', 'flexible-frontend-login' )));
      die();
    }
  }
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = isset($this->_saved[$field['id']])? $this->_saved[$field['id']]: '';
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '" alt="'.__('Remove', 'flexible-frontend-login' ).'" title="'.__('Remove', 'flexible-frontend-login' ).'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '" alt="'.__('Add', 'flexible-frontend-login' ).'" title="'.__('Add', 'flexible-frontend-login' ).'" id="add-'.$jsid.'"><br/></div>';
    ...
    echo '" alt="'.__('Remove', 'flexible-frontend-login' ).'" title="'.__('Remove', 'flexible-frontend-login' ).'" id="remove-'.$jsid.'"></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_plupload( $field, $meta) {  
    ...
    $html = '
        ...
        <div class="plupload-upload-uic hide-if-no-js '.$m1.'" id="'.$id.'plupload-upload-ui">
          <input id="'.$id.'plupload-browse-button" type="button" value="'.__('Select Files', 'flexible-frontend-login' ).'" class="button" />
  ...
  public function show_field_file( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      if ( ! empty( $meta ) ) {
        $nonce = wp_create_nonce( 'at_ajax_delete' );
        echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files', 'flexible-frontend-login' ) . '</strong></div>';
        echo '<ol class="at-upload">';
        foreach ( $meta as $att ) {
          // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
          echo "<li>" . wp_get_attachment_link( $att, '' , false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}|{$post->ID}|{$field['id']}|{$att}'>" . __( 'Delete', 'flexible-frontend-login' ) . "</a>)</li>";
        }
        echo '</ol>';
      }
      ...
      echo "<div class='at-file-upload-label'>";
        echo "<strong>" . __( 'Upload new files', 'flexible-frontend-login'  ) . "</strong>";
      echo "</div>";
      echo "<div class='new-files'>";
        echo "<div class='file-input'>";
          echo "<input type='file' name='{$field['id']}[]' />";
        echo "</div><!-- End .file-input -->";
        echo "<a class='at-add-file button' href='#'>" . __( 'Add more files', 'flexible-frontend-login'  ) . "</a>";
  ...
  public function show_field_typo( $field, $meta ) {
    ...
    $html .= "<input type='button' class='at-color-select button' rel='".$field['id']."[color]' value='" . __( 'Select a color', 'flexible-frontend-login'  ) . "'/>";
  ...
  public function show_field_color( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color', 'flexible-frontend-login'  ) . "'/>";
  ...
  public function show_import_export(){
    ...
    <div class="apc_ie_panel field">
      <div style="padding 10px;" class="apc_export"><h3>'.__('Export', 'flexible-frontend-login' ).'</h3>
        <p>'. __('To export saved settings click the Export button bellow and you will get the export Code in the box bellow, which you can later use to import.', 'flexible-frontend-login' ).'</p>
        <div class="export_code">
          <label for="export_code">'. __('Export Code', 'flexible-frontend-login' ).'</label><br/>
          <textarea id="export_code"></textarea>        
          <input class="button-primary" type="button" value="'. __('Get Export', 'flexible-frontend-login' ).'" id="apc_export_b" />'.$this->create_export_download_link().'
      ...
      <div style="padding 10px;" class="apc_import"><h3>'.__('Import', 'flexible-frontend-login' ).'</h3>
        <p>'. __('To Import saved settings paste the Export output in to the Import Code box bellow and click Import.', 'flexible-frontend-login' ).'</p>
        <div class="import_code">
          <label for="import_code">'. __('Import Code', 'flexible-frontend-login' ).'</label><br/>
          <textarea id="import_code"></textarea>
                  <input class="button-primary" type="button"  value="'. __('Import', 'flexible-frontend-login' ).'" id="apc_import_b" />
  ...
  public function export(){
    ...
    if (!isset($_GET['group'])){
      $re['err'] = __('error in ajax request! (1)', 'flexible-frontend-login' );
    ...
    if ($options !== false)
      ...
      $re['err'] = __('error in ajax request! (2)', 'flexible-frontend-login' );
  ...
  public function import(){
    ...
    if (!isset($_POST['imp'])){
      $re['err'] = __('error in ajax request! (3)', 'flexible-frontend-login' );
    ...
    if (is_array($import_code)){
      ...
      $re['success']= __('Setting imported, make sure you ', 'flexible-frontend-login' ). '<input class="button-primary" type="button"  value="'. __('Refresh this page', 'flexible-frontend-login' ).'" id="apc_refresh_page_b" />';
    }else{
      $re['err'] = __('Could not import settings! (4)', 'flexible-frontend-login' );
    }
    //update_nonce
      $re['nonce'] = wp_create_nonce("apc_import");
    echo json_encode($re);
  ...
  public function download_file($content = null, $file_name = null){
    ...
    if ( !current_user_can('edit_themes') )
        wp_die('<p>'.__('You do not have sufficient permissions to edit templates for this site.', 'flexible-frontend-login' ).'</p>');
    
    if ($content === null || $file_name === null){
        wp_die('<p>'.__('Error Downloading file.', 'flexible-frontend-login' ).'</p>');     
    }
